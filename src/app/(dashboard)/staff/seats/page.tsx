...
export default async function StaffSeatsPage({
  ...
  const seatList = ((seats ?? []) as SeatRow[])
    ...
    .filter((seat) => {
      ...
      return (
        String(seat.seat_number).includes(query) ||
  ...
  const statusBadge = (status: string) => {
    const base = "rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-[0.22em]";
    if (status === "available") return `${base} bg-emerald-50 text-emerald-700 border border-emerald-100`;
    if (status === "blocked") return `${base} bg-amber-50 text-amber-700 border border-amber-100`;
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[2rem] border border-[#d8e0d4] bg-white p-6 shadow-lg shadow-[#27452e]/6">
        <p className="text-[11px] font-bold uppercase tracking-[0.32em] text-[#6d7c6c]">Seat Operations</p>
        <h1 className="mt-3 text-4xl font-black text-[#1b3022]">Block And Release Seats</h1>
      </section>
      ...
      {error ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {seatList.map((seat) => (
            <article key={seat.id} className="rounded-[1.8rem] border border-[#d8e0d4] bg-white p-5 shadow-lg shadow-[#27452e]/6">
              <div className="flex items-center justify-between gap-4">
                <p className="text-3xl font-black text-[#1b3022]">#{seat.seat_number}</p>
                <span className={statusBadge(seat.status)}>{seat.status}</span>
              </div>
              <div className="mt-4 space-y-1 text-sm text-[#556455]">
                <p>Student: {seat.student?.name ?? "None"}</p>
                <p>Reason: {seat.block_reason || "None"}</p>
              </div>

              {seat.status === "available" ? (
                <form action={blockSeatForEnquiry} className="mt-5 space-y-3">
                  ...
                  <PendingSubmitButton
                    ...
                    className="w-full rounded-2xl border border-[#d7ddd3] bg-white px-5 py-3 text-[11px] font-black uppercase tracking-[0.3em] text-[#1b3022] disabled:cursor-not-allowed disabled:opacity-70"
                  />
                </form>
              ) : seat.status === "blocked" ? (
                <form action={releaseSeat} className="mt-5">
                  <input type="hidden" name="seat_id" value={seat.id} />
                  <PendingSubmitButton
                    idleLabel="Release Block"
