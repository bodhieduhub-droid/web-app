...
export default async function SeatHistoryPage({
  params,
}: {
  ...
  if (!seat) {
    return (
      <div className="p-8 text-center">
        ...
        <Link href="/super-admin/seats" className="mt-4 inline-block text-sm text-[#1b3022] underline">
          Back to seats
        </Link>
      </div>
    );
            ...
            {usage && usage.length > 0 ? (
              usage.map((record) => (
                <tr key={record.id} className="transition hover:bg-[#fcfdfb]">
                  ...
                  <td className="px-6 py-4">
                    <Link
                      href={`/super-admin/students/${record.id}`}
                      className="text-xs font-black uppercase tracking-widest text-[#1b3022] hover:underline"
                    >
                      View
                    </Link>
                  </td>
              ...
              <tr>
                <td colSpan={5} className="px-6 py-12 text-center text-sm font-medium text-[#6d7c6c]">
