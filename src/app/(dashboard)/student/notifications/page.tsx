...
// Async component — loads notifications independently
async function NotificationsList({
  studentId, profileId, cursor,
}: { studentId: string; profileId: string; cursor: string | null }) {
  ...
  const all = paged.map((n) => ({ ...n, effective_read_at: readMap.get(n.id) ?? (n.audience_type !== "broadcast_role" ? n.read_at : null) }));
  ...
  return (
    <div className="space-y-3">
      ...
      {hasMore && nextCursor && (
        <div className="pt-2">
          <Link href={`/student/notifications?cursor=${encodeURIComponent(nextCursor)}`} prefetch={false} className="inline-flex rounded-2xl border border-[#d8e0d4] bg-white px-4 py-3 text-sm font-semibold text-[#1b3022]">
...
export default async function NotificationsPage({
  ...
  // unread badge: lightweight count only
  const unread = { placeholder: true };
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ── Notification List (SUSPENSE — streams independently) ── */}
      <Suspense fallback={<ListSkeleton rows={5} />}>
        <NotificationsList studentId={student.id} profileId={profile.id} cursor={cursor} />
      </Suspense>
    </div>
  );
}
