...
export function StudentHero({ studentName, seatNumber, streakCount = 0 }: StudentHeroProps) {
  ...
  return (
    <LocalStorageCache 
      ...
      data={{ studentName, seatNumber, streakCount, greeting }}
    >
      {(data) => {
        const d = data || { studentName, seatNumber, streakCount, greeting };
                ...
                <div className="mt-10 flex flex-wrap gap-4">
                  {d.seatNumber ? (
                    <div className="flex items-center gap-3 rounded-2xl bg-white/10 px-5 py-3 backdrop-blur-md border border-white/5">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm font-black text-white">{d.seatNumber}</p>
                      </div>
                    </div>
                  ) : (
                  ...
                  <div className="flex items-center gap-3 rounded-2xl bg-white/10 px-5 py-3 backdrop-blur-md border border-white/5">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm font-black text-white">{d.streakCount} Day Streak</p>
                    </div>
                  </div>
                </div>
              </div>
