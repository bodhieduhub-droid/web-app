...
export function AttendanceCard({ todayAttendance, streakCount }: AttendanceCardProps) {
  return (
    <div className="rounded-[2.4rem] border border-[#d8e0d4] bg-white p-6 shadow-xl shadow-[#27452e]/5 transition-all hover:shadow-[#27452e]/8">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        {/* Status Section */}
        <div className={`flex flex-col justify-center rounded-3xl p-5 border ${todayAttendance ? 'bg-[#f7faf6] border-[#e1eadc]' : 'bg-[#fffbeb] border-[#fef3c7]'}`}>
          <div className="flex items-center gap-3">
            <div className={`flex h-10 w-10 items-center justify-center rounded-2xl ${todayAttendance ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
              {todayAttendance ? <CheckCircle2 className="h-5 w-5" /> : <MapPin className="h-5 w-5" />}
            </div>
            <div>
              ...
              <p className="text-lg font-black text-[#1b3022]">
                {todayAttendance ? 'Checked In' : 'Not Present Yet'}
              </p>
            </div>
          </div>
            ...
            <div className="mt-4 space-y-2 border-t border-[#e1eadc] pt-4">
              ...
              {todayAttendance.check_out_at && (
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-black text-[#1b3022]">
                    {new Date(todayAttendance.check_out_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </span>
                </div>
              )}
            </div>
