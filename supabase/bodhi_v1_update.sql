-- Bodhi Edu Hub / Reading Hub v1 update
-- Apply this after backing up the existing database.
...
alter table if exists public.enquiries drop constraint if exists enquiries_status_check;
...
alter table if exists public.readers
  ...
  add column if not exists preparing_for_exam boolean not null default false,
  add column if not exists exam_details text,
  add column if not exists onboarding_completed boolean not null default false,
  add column if not exists onboarding_completed_at timestamptz,
  add column if not exists id_proof_public_id text,
  add column if not exists monthly_fee numeric not null default 1650,
  add column if not exists converted_from_enquiry_id uuid references public.enquiries(id) on delete set null,
  add column if not exists credentials_sent_at timestamptz,
  add column if not exists updated_at timestamptz not null default timezone('utc', now());

alter table if exists public.readers drop constraint if exists readers_status_check;
alter table if exists public.readers
...
set monthly_fee = 1650
where monthly_fee is null;

alter table if exists public.seats
  add column if not exists blocked_by_profile_id uuid references public.profiles(id) on delete set null,
  add column if not exists block_reason text,
  add column if not exists blocked_until timestamptz,
  add column if not exists linked_enquiry_id uuid references public.enquiries(id) on delete set null,
  add column if not exists updated_at timestamptz not null default timezone('utc', now());
...
alter table if exists public.bills
  add column if not exists invoice_kind text not null default 'monthly_renewal',
  ...
  add column if not exists caution_amount numeric not null default 0,
  add column if not exists prorated_days int,
  add column if not exists updated_at timestamptz not null default timezone('utc', now());

alter table if exists public.bills drop constraint if exists bills_invoice_kind_check;
alter table if exists public.bills
...
alter table if exists public.bills drop constraint if exists unique_monthly_bill;
alter table if exists public.bills
  add constraint unique_monthly_bill unique (reader_id, month, year, invoice_kind);

update public.bills
set invoice_kind = 'monthly_renewal'
...
alter table if exists public.transactions
  ...
  check (type in ('upi', 'refund', 'manual_adjustment', 'offline', 'monthly', 'weekly', 'daily', 'registration', 'caution'));

alter table if exists public.transactions drop constraint if exists transactions_payment_mode_check;
alter table if exists public.transactions
  add constraint transactions_payment_mode_check
  check (payment_mode in ('upi', 'online', 'cash', 'offline'));

alter table if exists public.transactions drop constraint if exists transactions_verification_status_check;
alter table if exists public.transactions
...
alter table if exists public.notifications drop constraint if exists notifications_audience_type_check;
...
alter table if exists public.posts drop constraint if exists posts_type_check;
...
alter table if exists public.posts drop constraint if exists posts_audience_check;
alter table if exists public.posts
  add constraint posts_audience_check
  check (audience in ('public', 'student'));

alter table if exists public.posts drop constraint if exists posts_status_check;
alter table if exists public.posts
